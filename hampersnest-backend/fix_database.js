...
async function run() {
  const queries = [
    // 1. Drop the wrong BLOB/mismatched columns
    ...
    // 2. Add them back as CLOB to match Sequelize's DataTypes.TEXT / DataTypes.JSON mapping
    { sql: 'ALTER TABLE "products" ADD "images" CLOB', ignoreError: 'ORA-01430' },
    { sql: 'ALTER TABLE "orders" ADD "history" CLOB', ignoreError: 'ORA-01430' },
    { sql: 'ALTER TABLE "inquiries" ADD "history" CLOB', ignoreError: 'ORA-01430' }
  ];

  console.log('Starting migration to change mismatched BLOB columns to CLOB...');
